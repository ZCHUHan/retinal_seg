...
def data_process(data_path, name, patch_size, stride, mode):
    ...
    if name == "DRIVE":
        ...
        img_path = os.path.join(data_path, mode, "stare-images")
        gt_path = os.path.join(data_path, mode, "labels-ah")
        file_list = list(sorted(os.listdir(img_path)))
    elif name == "DCA1":
    ...
    for i, file in enumerate(file_list):
        if name == "DRIVE":
            ...
            gt = Image.open(os.path.join(gt_path, file)).convert("L")
            img = Grayscale(1)(img)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-dp', '--dataset_path', default="data/DRIVE", type=str,
