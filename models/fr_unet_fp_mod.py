...
class conv(nn.Module):
    def __init__(
        ...
        # conv+bn
        self.conv = nn.Conv2d(out_channels, out_channels, kernel_size=kernel_size, padding=padding)
...
class FR_UNet_Quan(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.out_ave == True:
            output = self.fuse(self.final1(x1_1)+self.final2(x10) +
                      self.final3(x11)+self.final4(x12)+self.final5(x13))
        else:
