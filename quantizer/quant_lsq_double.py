...
class QuanConv(nn.Conv2d):
    ...
    def forward(self, input, scale_x=None):
        ...
        if self.training and self.norm: 
            output = F.conv2d(x.float(), self.weight, self.bias,
        ...
        weight_true_int = weight_integer / weight_scaling_factor
        weight_true_int = weight_true_int.to(x.device)
        weight_integer = weight_integer.to(x.device)
        weight_scaling_factor = weight_scaling_factor.to(x.device)
        
        if b != None:
            b = b.to(x.device)
            bias_true_int = SymmetricQuantFunction.apply(b, 16, scale_x * weight_scaling_factor.squeeze())
            bias_integer = bias_true_int * scale_x * weight_scaling_factor.squeeze()
            bias_integer = bias_integer.to(x.device)
            bias_true_int = bias_true_int.to(x.device)
        else:
            bias_integer = None
            bias_true_int = None

        x_true_int = x / scale_x
        x_true_int = x_true_int.to(x.device)
        # new to export onnx, part1
        if get_global_idx() == 0:
            np.save("npz_logging/input.npy",
                x_true_int.detach().cpu().numpy()
                )  
        
        #output2 = F.conv2d(x, weight_integer, bias_integer, self.stride, self.padding, self.dilation, self.groups) 
        output2int = F.conv2d(x_true_int.float(), weight_true_int, bias_true_int, self.stride, self.padding, self.dilation, self.groups)
        output2 = output2int.double() * (scale_x.double() * weight_scaling_factor.double().squeeze()).unsqueeze(1).unsqueeze(2)
        ...
        if not self.training and get_global_idx() >= 0: #log npz:
            ...
            if bias_integer != None:
                np.savez("npz_logging/" + str(idx) + "_conv",
                         ...
                         outputInt=output2int.detach().cpu().numpy()
                         )
            else:
                np.savez("npz_logging/" + str(idx) + "_conv",
                         w=weight_integer.detach().cpu().numpy(), 
